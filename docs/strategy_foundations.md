...
Executable substrates live in `quantkit/` and `galahad-futures/`; this note is
the research spine.

Evidence for the citations below was gathered via arXiv abs-page scrapes and
the arXiv API — not paywalled harvest.
...
It is **not** an LLM multi-agent that invents orders. LLM research agents
(e.g. TradingAgents) may inform human research; they do not
...
The paper book (`quantkit.book` / `galahad-futures`) applies leverage, fees, MTM, and liquidation.
...
Implementation lives in `quantkit.validation` (purged k-fold +
embargo, walk-forward, Deflated Sharpe Ratio, CSCV/PBO, block bootstrap).
Doctrine:
...
Scrape logs (2026-08-05) covered the arXiv abs pages above successfully; SSRN
abstract pages were not required for doctrine.

#!/usr/bin/env python3
"""01b_optimize.py — 优化器权重：读取选股结果，用指数增强优化器计算最优权重。

接线：01_select.py → 01b_optimize.py → 02_trade.py
...
def main() -> int:
    ...
    p = argparse.ArgumentParser(description="quant_desk 优化器权重")
    ...
    if not weights_path.exists():
        print("ERROR: target_weights.csv 不存在，先跑 01_select.py", file=sys.stderr)
        return 1
    ...
    if not meta_path.exists():
        print("ERROR: selection_meta.json 不存在", file=sys.stderr)
        return 1

    meta = json.loads(meta_path.read_text())
    print(f"选股结果：{len(symbols)} 只，等权 benchmark")
    ...
    if len(returns) < 2:
        print("ERROR: 可用标的不足2只，无法优化", file=sys.stderr)
        return 1

    ret_df = pd.DataFrame(returns).dropna()
    print(f"协方差矩阵：{ret_df.shape[0]} 天 × {ret_df.shape[1]} 只")
    ...
    try:
        ...
        print(f"优化失败，回退等权：{e}", file=sys.stderr)
        opt_w = benchmark_w
    ...
    print(f"优化完成：TE={te:.4f}，换手={turnover:.4f}")
    print(f"权重范围：[{opt_w.min():.3f}, {opt_w.max():.3f}]")
    ...
    print(f"已覆盖 target_weights.csv → {weights_path}")
    return 0

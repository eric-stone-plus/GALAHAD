...
def main() -> int:
    cfg = load_cfg()
    p = argparse.ArgumentParser(description="quant_desk 纸面调仓")
    p.add_argument("--reset", action="store_true", help="重置账本为初始资金")
    args = p.parse_args()
    ensure_layout()
    ...
    if not tw_path.exists():
        print("ERROR: 请先运行 01_select.py 生成 target_weights.csv", file=sys.stderr)
        return 1
    ...
    if not prices:
        print("ERROR: 无法获取价格", file=sys.stderr)
        return 1

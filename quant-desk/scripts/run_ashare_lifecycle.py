...
def main() -> int:
    ...
    print(f"Downloading universe of {len(universe)} names (via proxy) …")
    ...
    if prices.empty:
        print("ERROR: price panel is empty", file=sys.stderr)
        return 1

    print(f"Price panel: {len(prices)} rows × {len(prices.columns)} cols ({prices.columns.tolist()})")
    ...
    if dropped:
        print(f"WARN: skipping symbols with insufficient data: {dropped}")
    prices = prices[good_cols]
    if prices.empty:
        print("ERROR: price panel empty after filtering", file=sys.stderr)
        return 1

    cal = prices.loc[args.start:].index
    if len(cal) < 30:
        print("ERROR: backtest window too short", file=sys.stderr)
        return 1
    ...
    print(f"Gate verdict: {gate_report['verdict']} → {OUT / 'gate_report.json'}")
    ...
    notes = [
        f"Backtest start {args.start}, rebalance {args.rebalance}, selection {args.mode} Top-{args.top_n}",
        f"Universe {len(universe)} names; cost assumption cost_tier={cost_tier or 'legacy'} (single-side {fee_bps:g} bps)",
        f"Final holdings: {last_selected}",
        f"Total return {_pct(perf.total_return)} · CAGR {_pct(perf.cagr)} · Sharpe {perf.sharpe:.2f} · MaxDD {_pct(perf.max_drawdown)}",
        f"Gate verdict: {gate_report['verdict']} (fail-closed on missing evidence; see output/gate_report.json)",
        "Paper fills: rebalance at close prices; simple fee model beyond limit-up/limit-down and impact cost is ignored.",
    ]
    html = write_review_html(
        title=f"Lifecycle review · {cfg.get('name', 'quant_desk')}",

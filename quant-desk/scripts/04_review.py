...
def main() -> int:
    ...
    if eq is None or eq.empty:
        print("ERROR: 无权益曲线", file=sys.stderr)
        return 1
    ...
    notes = [
        f"策略台：{cfg.get('name', 'quant_desk')}，初始资金 {cfg.get('initial_cash')}",
        f"选股模式：{(cfg.get('selection') or {}).get('mode')}，Top-N={(cfg.get('selection') or {}).get('top_n')}",
        "本报告为研究/纸面链路复盘；实盘请接 vnpy_cloud_bridge 或券商接口。",
    ]
    if (OUT / "selection_meta.json").exists():
        meta = json.loads((OUT / "selection_meta.json").read_text(encoding="utf-8"))
        notes.insert(0, f"最近选股：{meta.get('selected')}（asof {meta.get('asof')}）")
    ...
    html = write_review_html(
        title=f"投研复盘 · {cfg.get('name', 'quant_desk')}",

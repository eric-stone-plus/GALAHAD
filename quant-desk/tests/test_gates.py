"""quant_desk 门控接线测试（scripts/_gates.py + _common.resolve_fee_bps）。

Contracts:
  - config 声明 cost_tier 后纸面单边费 = tier/2，禁「万五」默认；legacy fee_bps 仍可读
  - collect_metrics 只产出流水线可实测键（turnover/aum/friction/cost_tier），不伪造证据
  - 缺 fills/equity 或跨度过短 → turnover 缺测 → gate0 fail-closed
  - run_gate_report 写 gate_report.json；实测键优先于外部证据同名键
  - 外部证据补齐后 verdict 可 GO（fixtures/gate_metrics_go.json）
"""
...
def test_resolve_fee_bps_cost_tier_half_per_side():
    ...
    assert tier == "mid"
    assert fee == pytest.approx(20.0)  # 0.4% 双边 → 单边 20 bps
...
def test_collect_metrics_legacy_fee_maps_to_two_sided(tmp_path):
    ...
    assert m["friction_cost"] == pytest.approx(0.001)  # 万五单边 → 双边 0.1%（低于带宽，gate0 会拒）


def test_turnover_missing_without_artifacts_or_short_span(tmp_path):
    assert "turnover_annual" not in collect_metrics(CFG, tmp_path)  # 空目录
    _write_artifacts(tmp_path, span_days=7)  # 跨度太短 → 缺测而非给噪声数
...
def test_measured_keys_win_over_extra(tmp_path):
    ...
    # 实测 mid/0.004 与 fills 推算不被 stale 证据软化

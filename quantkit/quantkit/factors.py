...
# ---------------------------------------------------------------------------
# 华泰 unified cross-sectional preprocessing pipeline
# (docs/library_digest/finance_ml_models.md 可迁移清单 1 — 全部 7 篇华泰 AI
# 报告共用; 华泰21《基于遗传规划的选股因子挖掘》适应度管线)
# ---------------------------------------------------------------------------
...
def _ols_residual_per_date(
    ...
    行业市值中性化取残差 (华泰统一流水线第 3 步). Dates with too few valid
    names to regress are left as NaN (excluded downstream) rather than
    silently passed through un-neutralized.
    """
...
def preprocess_cross_section(
    ...
    """华泰统一因子预处理流水线, applied per date cross-section.

    Steps for each column of ``factor_cols`` (华泰 AI 系列全 7 篇共用流程,
    finance_ml_models.md 可迁移清单 1):

      1. **MAD 去极值** — clip at ``median ± mad_k · MAD`` with
         ``MAD = median(|x − median|)`` (报告口径 DM ± 5·DM1).
      2. **缺失值填充** — industry-date mean when ``industry_col`` is given,
         falling back to the date median (global fallback).
      3. **行业市值中性化** — OLS residual of
      ...
      4. **截面 zscore 标准化** (constant cross-section → 0, no information).
...
# (docs/library_digest/finance_factor_mining_models.md 可迁移清单 1 —
# AlphaNet/TFT 统一训练协议: 滚动窗口 + 半年滚动重训 + 10 种子等权集成)
# ---------------------------------------------------------------------------
...
class WFRankResult:
    ...
    RankIC std against the mean single-seed per-fold std (华泰纪律: 集成不仅
    指标最好, 且标准差最小 — finance_ml_models.md 可迁移清单 3).
    """
...
def train_lightgbm_ranker_walkforward(
    ...
    Training protocol (AlphaNet 三部曲 / TFT 统一协议, finance_factor_mining_
    models.md 可迁移清单 1): 原报告的 "1500 日窗口 / 半年滚动 / 10 种子等权"
    适配为 ``quantkit.validation.walk_forward_splits`` 的扩展窗 — 切分只落在
    月边界, 测试块每次滚动 ``val_months`` 个月 (≈半年), ``purge_bars`` 在
    训练集尾部留出隔离带防重叠标签泄漏 (update_ml_validation.md 双协议:
    purged walk-forward 作最终可部署 OOS). 每折训练 ``n_seeds`` 个不同
    ``random_state`` 的模型, OOS 预测取等权平均.

    标签应为**回归**标签 (标准化下期/超额收益), 不要用分类 (华泰系列 17:
    回归标签整体优于分类).
    ...
    for k, (tr_d, te_d) in enumerate(
        ...
        ens = sum(seed_pred.values()) / float(n_seeds)  # 种子等权集成

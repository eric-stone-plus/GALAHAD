"""Performance evaluation and period review."""
...
def write_review_html(
    *,
    ...
    contrib_html = "<p class='muted'>None</p>"
    if contribution is not None and not contribution.empty:
        cdf = contribution.rename("contribution").reset_index()
        cdf.columns = ["symbol", "contribution"]
        cdf["contribution"] = cdf["contribution"].map(_pct)
    ...
    notes = notes or []
    notes_li = "\n".join(f"<li>{n}</li>" for n in notes) or "<li class='muted'>None</li>"
...
<div class="wrap">
  <h1>{title}</h1>
  <div class="meta">Generated: {now} · Window: {perf['start']} → {perf['end']} · Trading days: {perf['n_days']}</div>
  <div class="disclaimer">Research / paper-trading review report. Data may be delayed. Not investment advice.</div>

  <div class="kpi">
    <div class="item"><div class="label">Total return</div><div class="val">{_pct(perf['total_return'])}</div></div>
    ...
    <div class="item"><div class="label">Max drawdown</div><div class="val">{_pct(perf['max_drawdown'])}</div></div>
    <div class="item"><div class="label">Vol (ann.)</div><div class="val">{_pct(perf['vol_ann'])}</div></div>
    <div class="item"><div class="label">Daily win rate</div><div class="val">{_pct(perf['win_rate'])}</div></div>
    <div class="item"><div class="label">Final equity</div><div class="val">{_num(perf['final_equity'], 0)}</div></div>
    <div class="item"><div class="label">Fills</div><div class="val">{tstats['n_fills']}</div></div>
  </div>
  ...
  <div class="card">
    <table class="tbl">
      ...
      <tr><td>Start equity</td><td>{_num(perf['start_equity'], 2)}</td></tr>
      <tr><td>Final equity</td><td>{_num(perf['final_equity'], 2)}</td></tr>
      <tr><td>Total return</td><td>{_pct(perf['total_return'])}</td></tr>
      <tr><td>CAGR</td><td>{_pct(perf['cagr'])}</td></tr>
      <tr><td>Sharpe</td><td>{_num(perf['sharpe'])}</td></tr>
      <tr><td>Max drawdown</td><td>{_pct(perf['max_drawdown'])}</td></tr>
      <tr><td>Vol (ann.)</td><td>{_pct(perf['vol_ann'])}</td></tr>
      {extra_rows}
    </table>
  </div>
  <div class="card">{eq_img or "<p class='muted'>No equity chart</p>"}</div>
  <div class="card">{dd_img or "<p class='muted'>No drawdown chart</p>"}</div>

  <h2>2. Holdings (latest)</h2>
  <div class="card">{df_html(hold_show)}</div>

  <h2>3. Fills (last 50)</h2>
  <div class="card">
    <p class="muted">Buy notional {_num(tstats['buy_notional'], 0)} · Sell notional {_num(tstats['sell_notional'], 0)} · Fees {_num(tstats['fees'], 2)}</p>
    {df_html(fill_show)}
  </div>

  <h2>4. Selection cross-section</h2>
  <div class="card">{df_html(selection)}</div>

  <h2>5. Return contribution (approx.)</h2>
  <div class="card">{contrib_html}</div>

  <h2>6. Review notes</h2>
  <div class="card"><ul>{notes_li}</ul></div>

  <footer>GALAHAD · quant_desk · quantkit.review</footer>
</div>
</body>
</html>
"""

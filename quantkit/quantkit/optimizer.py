...
- ``finance_factor_mining_models.md`` — GF Securities (广发) deep-learning
  research report 6 (2019):
  max ``score·w − λ·TC(w, w0)`` with ``TC = tc × ½‖w − w0‖₁`` (tc = 0.3%,
  single-side), s.t. tracking-error cap (annualized 7.75% for index-enhanced
  products), industry deviation ±10%, weighted size deviation ≤1%, ``w ≥ 0``,
  fully invested (``Σw = 1``), per-rebalance turnover hard cap ``½‖w − w0‖₁ ≤ 24%``.
...
- ``finance_factors_microstructure_timing.md`` — Xinghuo-8 (星火8, Caitong;
  covariance estimator comparison): ``Σ_shrink = α·F + (1−α)·S`` with
  structured target F; LW constant-correlation (``LW_ConstCoeff``) had the
  best GMV out-of-sample behaviour and is simple to implement.

Tracking-error annualization convention
----------------------------------------
GF Securities' 7.75% TE cap is ANNUALIZED (semi-monthly rebalance, 24 periods/yr).
...
_TC_RATE = 0.003  # Guangfa recipe: tc = three per thousand (single-side)
...
# ---------------------------------------------------------------------------
# Ledoit-Wolf-style shrinkage covariance (Xinghuo 8 recipe: Σ = αF + (1−α)S)
# ---------------------------------------------------------------------------
...
def lw_shrinkage_cov(
    ...
    *,
    ...
    method :
        ``constant_correlation`` (default; Xinghuo-8's ``LW_ConstCoeff`` — best
...
# ---------------------------------------------------------------------------
# Index-enhancement optimizer (Guangfa Deep Learning 6 recipe)
# ---------------------------------------------------------------------------
...
def index_enhanced_weights(
    ...
    """Maximize ``score·w − λ·tc·½‖w−w0‖₁`` s.t. GF Securities (广发) index-enhancement constraints.
    ...
    industry :
        ...
        ≤ ``industry_max_dev`` (GF Securities: ±10%).
    log_mktcap :
        Log market cap per asset; constraint on the weighted-average
        deviation ``|Σ(w_i − wb_i)·logcap_i|`` ≤ ``style_max_dev``
        (GF Securities: ≤1%; shift-invariant since weights sum to 1).
    te_max :
        Tracking-error cap, same frequency as ``cov`` (the GF Securities
        product cap 7.75% is annualized — convert, e.g. daily: ``0.0775/√252``).
    turnover_cap :
        Per-rebalance single-side turnover hard cap ``½‖w−w0‖₁`` (GF Securities: 24%).
        Ignored when ``prev_weights`` is None.
    prev_weights :
        Current holdings ``w0``. None ⇒ no turnover cap and no cost term.
    cost_lambda :
        Cost-penalty multiplier λ (GF Securities: λ=1 deducts real cost only
        and maximizes IR; λ=2 is counterproductive).
    tc_rate :
        Single-side transaction-cost rate (GF Securities: 0.3% = 0.003).
...
# ---------------------------------------------------------------------------
# λ sweep (Guangfa: IR peaks at λ=1, degrades at λ=2)
# ---------------------------------------------------------------------------
...
def lambda_sweep(
    ...
    **opt_kwargs: Any,
    ...
    period_returns :
        ...
        sweep frequency-agnostic (GF Securities rebalanced semi-monthly).
    periods_per_year :
    ...
    Note
    ----
    GF Securities ran the λ sweep WITHOUT the hard per-rebalance turnover cap
    (their annual turnover fell 18.6 → 15.1 → 13.9 for CSI1000 as λ rose
    0 → 1 → 2 — turnover was interior, limited by the cost penalty itself).
    The 24% hard
...
# ---------------------------------------------------------------------------
# Residual-Alpha attribution (Guangfa: 7 style + industry factors + residual)
# ---------------------------------------------------------------------------
...
def alpha_attribution(
    ...
    GF Securities deep-learning report 6 attribution: the per-period (excess)
    return is split into the part explained by 7 style factors + industry
    factors and a residual Alpha (their CSI1000 enhancement's excess is
    dominated by residual Alpha: per-period Alpha mean 0.73% / std 1.65%).
    ...
    benchmark_weights :
        ...
        ``(w − wb)′r`` is attributed (GF Securities attributes the excess).

    Returns
    -------
    AttributionResult with per-period totals and the summary keys
    ``residual_mean`` / ``residual_std`` (GF Securities' per-period Alpha
    mean/std), ``explained_share`` / ``residual_share`` of the summed
    absolute split.
    """

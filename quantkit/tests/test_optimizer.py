"""Tests for quantkit.optimizer — LW shrinkage covariance + 广发指增 optimizer.

Synthetic-data contracts (recipe sources:
docs/library_digest/finance_factor_mining_models.md — 广发深度学习研究报告6,
docs/library_digest/finance_factors_microstructure_timing.md — 星火8/财通 LW):

  - LW shrinkage pulls extreme sample correlations toward the constant-
    correlation target and stays well-conditioned when N > T (星火8: LW_ConstCoeff
  ...
  - the optimizer respects EVERY constraint on a 50-name panel: sum(w)=1,
    ...
    per-rebalance turnover ½‖w−w0‖₁ ≤ 24%-style cap (广发 recipe)
  - infeasible configs (TE vs turnover conflict) raise OptimizationError
    naming the binding constraints
  - λ sweep reproduces 广发's qualitative result on planted alpha+cost:
...
# ---------------------------------------------------------------------------
# lambda_sweep — 广发: IR peaks at λ=1
# ---------------------------------------------------------------------------


def _alpha_scenario(seed: int) -> tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
    """Planted persistent alpha + small observation noise + 千三 cost.

    Construction so the λ penalty differentiates (广发 mechanism):
    ...
    - the marginal small-Δ trades have gain < tc, so λ=0's churn loses money
      net of cost (left side; the modest side, as in 广发's own numbers).
    - turnover cap non-binding: 广发 ran the λ sweep with interior turnover
      (their 24%/rebalance hard cap was a separate experiment).
    """
    N, T = 40, 36
...
def test_lambda_sweep_ir_peaks_at_one():
    ...
    for seed in (7, 23):
        ...
        ir = tbl["ir_net"]
        # right side is the robust one (广发's dramatic effect: CSI300 IR 1.74→0.74)
        assert ir.loc[1.0] > ir.loc[2.0], f"seed {seed}: λ=2 should cut alpha: {ir.to_dict()}"
        # turnover strictly decreasing in λ (广发: 18.64→15.07→13.94 annual)
        to = tbl["avg_turnover"]
